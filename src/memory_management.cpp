...
void Buffer::Delete(int32_t count) //Deletes count characters to the right of the point if count is positive; to the left of the point if count is negative
{
	...
	else
	{
		if(count < 0)
		{
			...
			for(int32_t i=0; i<count; i++) *(point--) = 0; //delete the characters left of the point and move the point to the left
		}
		else if(count > 0)
		{
			for(int32_t i=0; i<count; i++) *(point++) = 0; //delete the characters right of the point
		}
	}	
}
